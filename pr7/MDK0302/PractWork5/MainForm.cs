...
namespace PractWork5
{
    public partial class MainForm : Form
    {
        ...
        int[] openKey;
        int[] privateKey;
        private void Encrypt(byte[] arr, string path)
        {
            try
            {
                ...
                if (IsSimple(p) && IsSimple(q))
                {
                    ...
                    byte[] asciiBytes = new byte[arr.Length * 2];
                    ...
                    File.WriteAllBytes(currentPathRes, asciiBytes);
                   
                }
                else
                {
                    MessageBox.Show("Числа не простые", "Ошибка");
                }
            }
            catch (Exception ex)
            {
        ...
        {
            byte[] asciiBytesGet = File.ReadAllBytes(currentPath);
            int[] massCiph = new int[asciiBytesGet.Length / 2];
            ...
            int symb;
            int siphCounter = 1;
            int[] arrCiph = new int[massCiph.Length];
            //дешифрование
            ...
            var asciiBytes = new byte[arrCiph.Length];
        ...
        private void EncryptButton_Click(object sender, EventArgs e)
        {
            ...
            var bytes = File.ReadAllBytes(OriginTextBox.Text);
            Encrypt(bytes, currentPath);
        ...
        private void PathButton_Click(object sender, EventArgs e)
        {
            ...
            {
                OriginTextBox.Text = currentPath = currentPathRes = dialog.FileName;
            }
        }
        string currentPath;
